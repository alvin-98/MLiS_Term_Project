...
def adjusted_rand_index(true_labels, pred_labels):
    ...
    ---For now, this is entrirely generated by o1. will have to recheck this function later.---
    ---bit it seems to works just fine---
    ...
    -------
    ari : float
        The Adjusted Rand Index, in the range [-1, 1]. Random (uniform) label 
        assignments yield an ARI close to 0. Perfectly matching labelings give 1. 
        Permuted but otherwise identical clusterings also give 1. 
        Negative values indicate less agreement than expected by chance.
    """

    # Convert inputs to arrays (just to be safe against lists, etc.)
    ...
    # 1. Identify unique clusters and map them to integer indices
